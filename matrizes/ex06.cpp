...
int main(){
    ...
    for (linha=0; linha<TAM; linha++){
        for (coluna=0; coluna<TAM; coluna++){
            soma += matriz[linha][coluna]; // vai percorrer cada coluna, e pegar o primeiro elemento da linha
        }
        vetor[linha] = soma; //o vetor na posição [1][2][3]... vai armazenar a soma total da linha
        soma = 0; // antes de passar pra proxima linha vai zerar a soma pra pegar a soma da proxima linha
    }
