...
int main()
{
    ...
    //FIM: COMANDOS PARA QUE O CURSOR N�O FIQUE PISCANDO NA TELAsrand(time(NULL));
    ...
    while (true) { //esse la�o faz o jogo rodar para sempre
        if (game_over == 1) {
            ...
            while(obstaculo_y<20){
                ...
                coord.Y = obstaculo_y;
                SetConsoleCursorPosition(GetStdHandle(STD_OUTPUT_HANDLE), coord);
                if(obstaculo_y<al || obstaculo_y>al+5){ //tamanho do buraco entre os pipes superior e inferior
                    cout<<char(219);
                    posYB = obstaculo_y;
                }
            ...
            while(obstaculo_y2<20){
                ...
                SetConsoleCursorPosition(GetStdHandle(STD_OUTPUT_HANDLE), coord);
                if(obstaculo_y2<al2 || obstaculo_y2>al2+5){ //tamanho do buraco entre os pipes superior e inferior
                    cout<<char(219);
                }
            ...
            if (bird_x  == obstaculo_x && (bird_y >= al && bird_y <= al+5)){ //se a coordenada x do passaro for igual a coordenada x do obstaculo e se a coordenada y  e x do passaro estiverem entre o vão dos pipes, marca 1 ponto
            ...
            if (bird_x == obstaculo_x2 && (bird_y >= al2 && bird_y <= al2+5)){ //se a coordenada x do passaro for igual a coordenada x do obstaculo e se a coordenada y  e x do passaro estiverem entre o vão dos pipes, marca 1 ponto
                pontos += 1; // Se o passaro passar no vão entre os pipes, acrescenta 1 ponto
                if (pontos >= 5) {
                    velocidade_jogo -= 10;
                }
